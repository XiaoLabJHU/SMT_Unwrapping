function varargout = TraceRotate3D(varargin)
%TRACEROTATE3D M-file for TraceRotate3D.fig
%      TRACEROTATE3D, by itself, creates a new TRACEROTATE3D or raises the existing
%      singleton*.
%
%      H = TRACEROTATE3D returns the handle to a new TRACEROTATE3D or the handle to
%      the existing singleton*.
%
%      TRACEROTATE3D('Property','Value',...) creates a new TRACEROTATE3D using the
%      given property value pairs. Unrecognized properties are passed via
%      varargin to TraceRotate3D_OpeningFcn.  This calling syntax produces a
%      warning when there is an existing singleton*.
%
%      TRACEROTATE3D('CALLBACK') and TRACEROTATE3D('CALLBACK',hObject,...) call the
%      local function named CALLBACK in TRACEROTATE3D.M with the given input
%      arguments.
%
...
% Edit the above text to modify the response to help TraceRotate3D

% Last Modified by GUIDE v2.5 11-Jan-2018 22:52:10

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @TraceRotate3D_OpeningFcn, ...
                   'gui_OutputFcn',  @TraceRotate3D_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
% --- Executes just before TraceRotate3D is made visible.
function TraceRotate3D_OpeningFcn(hObject, eventdata, handles, varargin)
...
% UIWAIT makes TraceRotate3D wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = TraceRotate3D_OutputFcn(hObject, eventdata, handles)
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if ~isfield(handles,'SRadius')
    errordlg('Analysis the septum first!');
    return;
end
if ~isfield(handles,'CRadius')
    errordlg('Analysis the cell first!');
    return;
end

% TraceInfo.TrackcOR_unwrap = handles.TrackcOR_unwrap;
TraceInfo.TrackcOR = handles.TrackcOR;
TraceInfo.Center = handles.Center;
TraceInfo.Radius = handles.SRadius;
TraceInfo.BFrot = handles.BFRot;
TraceInfo.CRadius = handles.CRadius;
...
% --- Executes on button press in left_cell_tag.
function left_cell_tag_Callback(hObject, eventdata, handles)
% hObject    handle to left_cell_tag (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
     XfineCell = handles.XfineCell ;
     XpixelCell = handles.XpixelCell;
     ScanLineCell = handles.ScanLineCell;
     ScanFineCell = handles.ScanFineCell;
     
     % select the region
     axes(handles.LineScan_tag);
     [X Y] = ginput(2);
     Index = find(XpixelCell > min(X) & XpixelCell < max(X));
     MeanLeft = mean(ScanLineCell(Index));
     
     % plot the line
     hold on
     plot(XfineCell,ones(size(XfineCell))*MeanLeft,'b');
     handles.MeanLeftCell = MeanLeft;
guidata(hObject,handles);

% --- Executes on button press in centercell_tag.
function centercell_tag_Callback(hObject, eventdata, handles)
% hObject    handle to centercell_tag (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
axes(handles.BFimage_tag);
[X Y] = ginput(1);
...
BFImage = handles.BFRot;
% calculate the profile on the horizatal line and spline to a finer curve
ScanLineCell = BFImage(round(Y),:);
XpixelCell = [1:size(BFImage,2)];
XfineCell = linspace(1,size(BFImage,2),size(BFImage,2)*10);
ScanFineCell = spline(XpixelCell,ScanLineCell,XfineCell); % smooth 10 folds by 
% plot the line out
axes(handles.LineScan_tag);
hold off
plot(XpixelCell,ScanLineCell,'ob');
hold on
plot(XfineCell,ScanFineCell,'-r','LineWidth',3);
% save the curves
handles.XfineCell = XfineCell;
handles.XpixelCell = XpixelCell;
handles.ScanLineCell = ScanLineCell;
handles.ScanFineCell = ScanFineCell;
guidata(hObject,handles);

% --- Executes on button press in Dcellgen_tag.
function Dcellgen_tag_Callback(hObject, eventdata, handles)
% hObject    handle to Dcellgen_tag (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% generate the unwrapped coordinates
     XfineCell = handles.XfineCell;
...
% line, when the image is not great or cells packed together
     ...
     % left point
     PosiLeft = min(X);
     Index1 = find(XfineCell <= PosiLeft);
     IndexLeft = Index1(end);
     
     % right point
     PosiRight = max(X);
     Index2 = find(XfineCell >= PosiRight);
     IndexRight = Index2(1);
     
     
     PixelS = handles.pixelS;
     Offset = 45/PixelS; % the real boundary is inward of the position from test
     
     % get the cell outline
     CellOut(1,1) = XfineCell(IndexLeft) + Offset;
     CellOut(1,2) = ScanFineCell(IndexLeft); % first row, the position and intensity of the left point
     CellOut(2,1) = XfineCell(IndexRight) - Offset;
     CellOut(2,2) = ScanFineCell(IndexRight);% second row, the position and intensity of the right point
     CellOut(3,1) = (XfineCell(IndexLeft) + XfineCell(IndexRight))/2; 
     CellOut(3,2) = mean(max(ScanFineCell));% third row, the position and intensity of the center of the cell
     
     handles.CellOut = CellOut;
     handles.CellD = CellOut(2,1) - CellOut(1,1);
     CRadius = (CellOut(2,1) - CellOut(1,1))/2;
     handles.CRadius = CRadius; % The radius with std of the estimation
     % label the center point
     Center(1) = CellOut(3,1);
     Center(2) = handles.CenterY;
     handles.CenterCell = Center;
     % plot the points out
     ...
     axes(handles.BFimage_tag);
     hold on
     plot(CellOut(1,1)*ones(Y),[1:Y],'-y');
     plot(CellOut(2,1)*ones(Y),[1:Y],'-k');
     plot(CellOut(3,1)*ones(Y),[1:Y],'-g');
     
     % Show the result on the panel
     OutputS{1,1} = ['Cell radius = ' num2str(CRadius) ' pixel'];
%      OutputS{2,1} = ['Septum radius = ' num2str(SRadius(2)) ' pixel'];
%      OutputS{3,1} = ['Septum radius = ' num2str(SRadius(2)*handles.pixelS) ' nm'];
%      OutputS{4,1} = ['Septum radius in a range from ' num2str(SRadius(3)) ' to ' num2str(SRadius(1)) ' pixel'];
     set(handles.cRadius_tag, 'String', OutputS);
...
% --- Executes on button press in rightcell_tag.
function rightcell_tag_Callback(hObject, eventdata, handles)
% hObject    handle to rightcell_tag (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
     XfineCell = handles.XfineCell ;
     XpixelCell = handles.XpixelCell;
     ...
     [X Y] = ginput(2);
     Index = find(XpixelCell > min(X) & XpixelCell < max(X));
     MeanRight = mean(ScanLineCell(Index));
     
     % plot the line
     hold on
     plot(XfineCell,ones(size(XfineCell))*MeanRight,'g');
     handles.MeanRightCell = MeanRight;
guidata(hObject,handles);
