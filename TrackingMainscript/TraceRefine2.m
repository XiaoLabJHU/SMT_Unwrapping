function varargout = TraceRefine2(varargin)
% TRACEREFINE2 MATLAB code for TraceRefine2.fig
%      TRACEREFINE2, by itself, creates a new TRACEREFINE2 or raises the existing
%      singleton*.
%
%      H = TRACEREFINE2 returns the handle to a new TRACEREFINE2 or the handle to
%      the existing singleton*.
%
%      TRACEREFINE2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in TRACEREFINE2.M with the given input arguments.
%
%      TRACEREFINE2('Property','Value',...) creates a new TRACEREFINE2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before TraceRefine2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to TraceRefine2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help TraceRefine2

% Last Modified by GUIDE v2.5 14-Jul-2021 11:43:00

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @TraceRefine2_OpeningFcn, ...
                   'gui_OutputFcn',  @TraceRefine2_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before TraceRefine2 is made visible.
function TraceRefine2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to TraceRefine2 (see VARARGIN)

% Choose default command line output for TraceRefine2
handles.output = hObject;
handles.pxSize = 100;
% Update handles structure
guidata(hObject, handles);

% UIWAIT makes TraceRefine2 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = TraceRefine2_OutputFcn(hObject, eventdata, handles) 
...
cd(handles.DT_dir);
handles.DT_imgs = dir('*.tif');
cd(handles.trace_dir);
handles.traces = dir('*.mat');
%conditional
handles.num_imgs = length(handles.BF_imgs);
...
if show_drift
    ...
    img_name = handles.DT_imgs(val).name;
    %IMG = imread(handles.DT_name, 'tif');
else
    cd(handles.BF_dir);
    %handles.BF_name = handles.BF_imgs(val).name;
    img_name = handles.BF_imgs(val).name;
    %IMG = imread(handles.BF_name, 'tif');
end
IMG = imread(img_name, 'tif');
axes(handles.image);
imshow(IMG,[]);
hold all

set(handles.BF_name,'String', img_name);
% save variable 
handles.ImBF=IMG;
handles.BFpath=pwd;
